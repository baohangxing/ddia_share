...
// Phase 2: Two users racing for 1 item
const raceNodes = [
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        {phase >= PHASE.INVENTORY && (
          <motion.div
            ...
            className="mb-8"
          >
            <InventoryCounter remaining={inventory} animateCrash={inventoryCrash} />
          </motion.div>
        )}

        {/* ===== 3. Race Condition: Two users buy simultaneously ===== */}
        <AnimatePresence>
          {phase >= PHASE.RACE && (
            <motion.div
              initial={{ opacity: 0, y: 30 }}
        ...
        <AnimatePresence>
          {phase >= PHASE.RAFT_LEADER && (
            ...
            >
              ...
              <ArchitectureDiagram
                nodes={raftElectionNodes.map((n) => {
                  // Turn followers green when majority is reached
                  if (majorityGreen && (n.id === 'f1' || n.id === 'f2')) {
                    return { ...n, color: 'success' as const };
                  }
                  if (majorityGreen && n.id === 'f3') {
                    return { ...n, color: 'danger' as const };
                  }
                  return n;
                })}
              ...
              <AnimatePresence>
                {majorityGreen && (
                  ...
                  >
                    <span className="text-xs font-mono text-[#4ade80] bg-[#22c55e15] px-3 py-1 rounded-full">
                      ✅ 多数派达成！获得 2/3 票 → 当选 Leader
                    </span>
                  </motion.div>
                )}
              </AnimatePresence>

              {/* Leader election detail text */}
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
        ...
        <AnimatePresence>
          {phase >= PHASE.LOG_REPLICATION && (
            ...
            >
              ...
              <ArchitectureDiagram
                nodes={logReplicationNodes.map((n) => {
                  if (logMajorityGreen && (n.id === 'flr1' || n.id === 'flr2')) {
                    return { ...n, color: 'success' as const };
                  }
                  return n;
                })}
              ...
              <AnimatePresence>
                {logMajorityGreen && (
                  ...
                  >
                    <span className="text-xs font-mono text-[#4ade80] bg-[#22c55e15] px-3 py-1 rounded-full">
                      ✅ 多数派确认 → Committed → 状态机执行
                    </span>
                  </motion.div>
                )}
              </AnimatePresence>

              {/* Log replication detail text */}
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.5 }}
