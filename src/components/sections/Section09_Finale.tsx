...
// ── Full architecture: all 10 nodes with their final positions ──
const ALL_NODES = [
...
// ── Arrows connecting the full architecture ──
const ALL_ARROWS = [
...
// ── Which node IDs are visible at each architecture phase ──
const PHASE_NODES: Record<number, string[]> = {
  [PHASE.SIMPLE]: ["user", "app", "store"],
  [PHASE.LOAD_BALANCER]: ["user", "lb", "app", "store"],
  [PHASE.CACHE_KAFKA]: ["user", "lb", "app", "cache", "kafka", "store"],
  [PHASE.SHARDING]: ["user", "lb", "app", "cache", "kafka", "shard", "store"],
  [PHASE.REPLICATION]: [
    "user",
    "lb",
    "app",
...
// ── Six keywords with their DDIA chapter references ──
const KEYWORDS = [
...
/** Filter nodes and arrows to only include the given node IDs */
...
/** A single glowing keyword + ChapterBadge */
...
const PHASE_SUBTITLES: Record<number, string> = {
  [PHASE.SIMPLE]: "最初的样子",
  [PHASE.LOAD_BALANCER]: "流量来了",
  [PHASE.CACHE_KAFKA]: "缓存 + 消息队列",
  [PHASE.SHARDING]: "分片扩展",
  [PHASE.REPLICATION]: "冗余副本",
  [PHASE.TRANSACTIONS]: "保证正确",
  [PHASE.CONSENSUS]: "达成共识",
  [PHASE.STORAGE]: "持久化根基",
};
...
export default function Section09_Finale() {
  ...
  const diagram = PHASE_NODES[phase]
    ? filterDiagram(PHASE_NODES[phase])
    : filterDiagram(["user", "app", "store"]);

  const showDiagram = phase >= PHASE.SIMPLE;
  const showQuote = phase >= PHASE.QUOTE;
  ...
  return (
    ...
    >
      ...
      <div className="max-w-3xl mx-auto relative z-10 w-full">
        ...
        {/* ===== 3. Architecture Diagram — remounts on each phase change ===== */}
        ...
        {/* ===== 4. Phase subtitle during architecture growth ===== */}
        <AnimatePresence>
          {showDiagram && phase < PHASE.QUOTE && (
            <motion.p
              key={`sub-${phase}`}
              ...
              className="text-center text-xs text-[#555] font-mono tracking-widest uppercase"
            >
              {PHASE_SUBTITLES[phase] ?? "架构演进中..."}
            </motion.p>
          )}
        </AnimatePresence>
        ...
        {/* ===== 6. Keywords grid with glow animation ===== */}
        <AnimatePresence>
          {phase >= PHASE.KEYWORD_1 && (
            <motion.div
              initial={{opacity: 0}}
              animate={{opacity: 1}}
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 max-w-xl mx-auto">
                {KEYWORDS.map((kw, i) => (
                  <AnimatePresence key={kw.label}>
                    {phase >= PHASE.KEYWORD_1 + i && (
                      <GlowKeyword
                        label={kw.label}
                        chapter={kw.chapter}
                        title={kw.title}
